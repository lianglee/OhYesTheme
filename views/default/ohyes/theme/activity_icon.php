 ...
 */
if(!isset($vars['image_size'])){
  $size_image = 'medium';	
} else {
   $size_image = $vars['image_size'];	
}
if(!isset($vars['attr_pic'])){
  $attr_pic = array(60, 60);	
} else {
   $attr_pic = explode('/', $vars['attr_pic']);	
}
if(!isset($vars['pic_class'])){
  $pic_class = 'ohyes-river-icon';	
} else {
   $pic_class = $vars['pic_class'];	
}
$user = elgg_extract('entity', $vars, $vars['user']);
$size = elgg_extract('size', $vars, $size_image);
